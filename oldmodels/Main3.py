...
name = "CR_M2_" + str(int(total_steps/1e6)) + "_" + str (num_levels) + "lvls_" + use_backgrounds*"bg_" + "DA"
...
encoder = Encoder2(img_inp_layers,enc_out_dim)
pos_encoder = PositionalEncoder(enc_out_dim,hor_splits*vert_splits)
transformer_attention = MultiHeadAttention(MHA_heads, enc_out_dim)
transformer_block = TransformerBlock(transformer_attention, enc_out_dim, transf_dropout, forward_scaling)
data_augmentation = DataAugmentation(brightness, p_bright, contrast, p_contr, saturation, p_satur, hue, p_hue, augment_prob)
policy = Policy(image_split, encoder, pos_encoder, transformer_block, vert_splits*hor_splits*enc_out_dim, env.action_space.n)
policy.cuda()
...
torch.save(policy.state_dict, 'D:/OneDrive - Danmarks Tekniske Universitet/Studie/5. Semester/Deep Learning/Projekt/Model1_states/' + name + '.pt')

import imageio

# Make evaluation environment
eval_env = make_env(n_envs=n_envs, 
               env_name=env_name,
               num_levels=num_levels,
               start_level= start_level,
               use_backgrounds=use_backgrounds,
...
imageio.mimsave('D:/OneDrive - Danmarks Tekniske Universitet/Studie/5. Semester/Deep Learning/Projekt/Model1_states/' + name + '.mp4', frames, fps=15)