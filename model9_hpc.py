...
import pandas as pd
import math
from projectutils import make_env, Storage, orthogonal_init, BaseStorage, BaseStorage2
...
# policy.cuda()
policy = Policy4(image_split,encoder,pos_encoder_img,transformer_block_img,vert_splits*hor_splits*enc_out_dim_img,env.action_space.n)
policy = policy.cuda()
...
while step < total_steps:
    ...
    episode_steps_used = 0
    episode_dones = 0
    
    if use_num_dones == 1:
        storage.reset()
        while episode_dones < num_total_dones:
            # Use policy
    ...
    storage.num_steps = episode_steps_used
    ...
    # Update stats
    step += n_envs * episode_steps_used
    print(f'Step: {step}\tMean reward: {storage.get_reward()}')
    step_ns.append(step)
    mean_rewards.append(storage.get_reward())
    if storage.get_reward() > 15:
        use_num_dones = 1
